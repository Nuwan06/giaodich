...
// Đọc tất cả dữ liệu giao dịch từ tệp lịch sử giao dịch
    vector<tuple<string, string, string, int, string, string>> LichsuGiaodich() {
        vector<tuple<string, string, string, int, string, string>> Giaodich; // Vector để lưu trữ dữ liệu giao dịch
        ifstream inputFile(Filegiaodich);   // Mở tệp lịch sử giao dịch để đọ
        string line;
        while (getline(inputFile, line)) {
            stringstream ss(line);
...
// Quy trình: Hiển thị ví của người dùng, yêu cầu thông tin giao dịch, kiểm tra tính hợp lệ của ví và số dư,
    ...
    void chuyenDiem() {
        ...
        lsgd.emplace_back(maGD, maViNguon, maViDich, soDiem, tg, "thanh_cong");   // Thêm giao dịch mới vào lịch sử
